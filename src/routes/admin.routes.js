...
router.patch('/users/:userId/plan', auditLog('CHANGE_USER_PLAN'), async (req, res) => {
  ...
  try {
    ...
    const expiryVal = req.body?.plan_expires_at ? new Date(req.body.plan_expires_at) : null;
    await pool.query(`UPDATE users SET plan = $1, plan_expires_at = $3 WHERE id = $2`, [planResult.value, uid, expiryVal]);
    await pool.query(
